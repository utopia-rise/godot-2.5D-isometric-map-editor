...
func test_assert_map_init_values():
	...
	assert_eq(test_map.get_global_3D_position(), expected_position, "Isometric map initial position should be " + str(expected_position) + ".")
...
func test_should_add_remove_iso_positionable():
	...
	inner_pos.local_3D_position = Vector3(1, 1, 2)
	test_map.add_iso_positionable(inner_pos)
	assert_eq(inner_pos.get_parent(), test_map, "Should have added as child iso map.")
	assert_eq(test_map.get_positionable_at(inner_pos.local_3D_position), inner_pos, "inner_pos should be found at " + str(inner_pos.local_3D_position))
...
func test_should_not_add_iso_positionable_out_of_map():
	...
	var inner_pos = IsometricPositionable.new()
	inner_pos.local_3D_position = Vector3(2, 0, 0)
	test_map.add_iso_positionable(inner_pos)
	assert_eq(test_map.get_children().size(), 0, "Should not find any positionable as child in map.")
	inner_pos.local_3D_position = Vector3(0, 2, 0)
	test_map.add_iso_positionable(inner_pos)
	assert_eq(test_map.get_children().size(), 0, "Should not find any positionable as child in map.")
	inner_pos.local_3D_position = Vector3(0, 0, 2)
	test_map.add_iso_positionable(inner_pos)
	assert_eq(test_map.get_children().size(), 0, "Should not find any positionable as child in map.")
	inner_pos.queue_free()
...
func test_should_not_add_overlapping_iso_positionable():
	...
	overlapping_pos.local_3D_position = Vector3(2, 2, 2)
...
func test_assert_flatten_isometricmap():
	...
	second_in_inner.local_3D_position = Vector3(0, 0, 1)
	...
	third_inner.local_3D_position = Vector3(8, 0, 0)
	...
	assert_eq(positionable.get_global_3D_position(), Vector3(0, 0, 1), "Original positionable's position should not have changed")
	assert_eq(flatten_map.get_node("posi").local_3D_position, Vector3(5, 0, 1), "Positionable should have a sum of its former position and its former parent.")
